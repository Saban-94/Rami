...
export default function AvailabilityCalendarBlock({ content, layout, data }: any) {
  ...
  // הגנה: אם content לא הגיע, נשתמש בערכי ברירת מחדל כדי למנוע קריסה
  const safeContent = {
    title: content?.title || "טוען יומן...",
    subtitle: content?.subtitle || "מיד נציג זמינות"
  };
  ...
  // אם המידע בטעינה קריטית, נציג Skeleton קטן במקום לקרוס
  if (!content && !data) {
     return <div className="p-10 animate-pulse bg-white/20 rounded-[28px] border border-white/10" />;
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        {/* Header - שימוש ב-safeContent המוגן */}
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-xl font-black italic tracking-tighter">{safeContent.title}</h3>
