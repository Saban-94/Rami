...
import { 
  LayoutDashboard, Database, Eye, Send, Sparkles, X, 
  BrainCircuit, User, Calendar 
} from "lucide-react";
...
export default function NielappStudioPage() {
  ...
  // כאן הייתה השגיאה - LoadingScreen חייבת להיות מוגדרת למטה
    ...
    <div className="h-screen bg-[#020617] text-white flex overflow-hidden font-sans" dir="rtl">
      ...
      <div className="flex-1 bg-[#0f172a] relative flex items-center justify-center">
        <div className="w-[340px] h-[680px] bg-white rounded-[3.5rem] border-[12px] border-slate-800 shadow-2xl relative overflow-hidden flex flex-col">
          ...
          <TemplateEngine manifest={manifest} trialId={trialId} />
          ...
          <div className="p-4 bg-white/80 backdrop-blur-md border-t border-slate-100 relative z-20">
            <div className="flex gap-2">
              <input 
                ...
                onKeyDown={e => e.key === 'Enter' && !isProcessing && (sendAnswer(input), setInput(""))}
                className="flex-1 bg-slate-100 rounded-xl p-3 text-xs text-slate-900 outline-none focus:ring-2 ring-blue-500 transition-all" 
...
/* --- תתי-קומפוננטות באותו קובץ למניעת שגיאות Reference --- */
    ...
    <div className="h-full flex flex-col">
      <div className="p-6 border-b border-white/10 flex items-center justify-between sticky top-0 bg-[#020617] z-10">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-black text-[10px] uppercase tracking-widest">Intelligence</h2>
        </div>
        <div className="flex bg-white/5 p-1 rounded-lg border border-white/10">
          <button onClick={() => setViewMode('visual')} className={`p-2 px-3 rounded-md text-[10px] ${viewMode === 'visual' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><Database size={14}/></button>
          <button onClick={() => setViewMode('raw')} className={`p-2 px-3 rounded-md text-[10px] ${viewMode === 'raw' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><Eye size={14}/></button>
        </div>
      </div>

      <div className="flex-1 p-4 space-y-4 overflow-y-auto">
        {viewMode === 'raw' ? (
          <pre className="p-4 bg-black/40 rounded-2xl text-[10px] text-blue-300 font-mono overflow-auto">
            {JSON.stringify(manifest, null, 2)}
          </pre>
        ) : (
          <div className="space-y-4">
             {proposal && (
               <div className="p-5 bg-blue-600 rounded-[2rem] border border-white/20 shadow-xl">
                 ...
                 <div className="flex gap-2">
                   ...
                   <button onClick={reject} className="p-3 bg-black/20 rounded-xl text-white"><X size={14}/></button>
                 </div>
               </div>
             )}

             <div className="p-5 bg-white/5 rounded-[2rem] border border-white/5">
                <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-4 flex items-center gap-2"><User size={12}/> Customers</h4>
...
// ההגדרה שהייתה חסרה וגרמה לשגיאה
function LoadingScreen() {
  return (
    <div className="h-screen bg-[#020617] flex flex-col items-center justify-center gap-6">
      <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin" />
      <p className="text-blue-500 font-black text-[10px] tracking-[0.3em] uppercase animate-pulse">
        Initializing SabanOS Brain...
      </p>
    </div>
  );
}
