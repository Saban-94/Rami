...
export default function SabanOSProductionStudio({ params }: { params: { trialId: string } }) {
  ...
  const [selectedStaff, setSelectedStaff] = useState("ammar");
  ...
  const handleVerify = () => {
    if (inputCode === businessData?.accessCode) {
      ...
      typeToCanvas(`מערכת SabanOS Studio v2.5 באוויר. שלום ${businessData?.businessName || 'עמאר'}. המוח מוכן לעיצוב.`);
    } else {
  ...
  const triggerSuccess = () => {
    ...
    typeToCanvas("התור שוריין בהצלחה! שלחתי ללקוח אישור בוואטסאפ ועדכנתי לך את היומן.");
  };
        ...
        <div className="lg:col-span-3 space-y-6 overflow-y-auto custom-scrollbar">
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[3.5rem] p-8 shadow-sm">
            <h2 className="text-xl font-black italic mb-8 flex items-center gap-2">
              <Layout size={20} className="text-green-500" /> ספרית תבניות
            </h2>
            <div className="space-y-2">
              {CATEGORIES.map((cat) => (
                <button key={cat.id} onClick={() => { setSelectedCat(cat); typeToCanvas(`הזרקתי את חבילת ה-${cat.name}. בוא נראה איך היא נראית באייפון.`); }} 
          ...
          <button onClick={() => setIsDarkMode(!isDarkMode)} className="w-full py-6 bg-slate-900 dark:bg-white text-white dark:text-black rounded-[2.5rem] font-black flex items-center justify-center gap-3 shadow-xl">
            {isDarkMode ? <Sun size={20}/> : <Moon size={20}/>} {isDarkMode ? 'SWITCH TO LIGHT' : 'SWITCH TO DARK'}
          </button>
        </div>

        {/* --- 2. Center: iPhone Live Canvas --- */}
        <div className="lg:col-span-5 flex flex-col items-center justify-center relative bg-slate-200/20 dark:bg-black/20 rounded-[5rem] border border-dashed border-slate-300 dark:border-white/5">
          <div className="w-[320px] h-[640px] bg-black rounded-[4rem] border-[12px] border-slate-800 shadow-2xl relative overflow-hidden ring-4 ring-black/10">
            ...
            <div className={`w-full h-full p-6 pt-14 transition-all duration-700 overflow-y-auto ${isDarkMode ? 'bg-[#0b141a]' : 'bg-white'}`}>
              ...
              {/* מודול יומן פרימיום בתוך האייפון */}
              <div className="text-center mb-8">
                <h3 className={`text-2xl font-black italic ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{businessData?.businessName}</h3>
                <p className="text-[10px] opacity-40 uppercase tracking-widest">Book an Appointment</p>
              </div>
              ...
              <button disabled={!selectedSlot || isSuccess} onClick={triggerSuccess} 
                ...
                {isSuccess ? 'שוריין בהצלחה!' : `שריון ל-${selectedSlot || '...'}`}
              </button>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-20 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-t border-slate-100 dark:border-white/5 flex items-center justify-around px-4">
              <div className="flex flex-col items-center gap-1 text-green-500"><Smartphone size={18}/><span className="text-[8px] font-black uppercase">App</span></div>
              <div className="flex flex-col items-center gap-1 opacity-20"><Scissors size={18}/><span className="text-[8px] font-black uppercase">Shop</span></div>
              <div className="flex flex-col items-center gap-1 opacity-20"><User size={18}/><span className="text-[8px] font-black uppercase">Profile</span></div>
            </div>
          </div>
        </div>

        {/* --- 3. Right: AI Designer Canvas --- */}
          ...
          <div className="bg-white dark:bg-[#0b141a] border border-slate-200 dark:border-white/10 rounded-[4rem] p-10 flex-1 flex flex-col shadow-2xl relative overflow-hidden">
            <div className="flex items-center gap-4 mb-10">
              ...
              <div>
                ...
                <p className="text-[10px] text-green-600 font-black uppercase mt-1">Live Sync Active</p>
              </div>
            </div>
